    ...
    <div class="bg-white shadow-sm border-b border-gray-100 py-4 px-6 mb-6">
        <div class="max-w-7xl mx-auto flex flex-col sm:flex-row justify-between items-start sm:items-center w-full gap-4 sm:gap-0" dir="{{ app()->getLocale() == 'ar' ? 'rtl' : 'ltr' }}">
            ...
            <div class="flex flex-wrap justify-start sm:justify-end items-center gap-2 sm:gap-4 w-full sm:w-auto">
                ...
                <a href="{{ route('apps') }}" class="flex items-center gap-2 px-3 py-2 bg-indigo-50 hover:bg-indigo-100 text-indigo-700 rounded-xl transition border border-indigo-100">
                    <svg class="w-4 h-4" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M4 6a2 2 0 012-2h2a2 2 0 012 2v2a2 2 0 01-2 2H6a2 2 0 01-2-2V6zM14 6a2 2 0 012-2h2a2 2 0 012 2v2a2 2 0 01-2 2h-2a2 2 0 01-2-2V6zM4 16a2 2 0 012-2h2a2 2 0 012 2v2a2 2 0 01-2 2H6a2 2 0 01-2-2v-2zM14 16a2 2 0 012-2h2a2 2 0 012 2v2a2 2 0 01-2 2h-2a2 2 0 01-2-2v-2z"></path></svg>
                    <span class="text-sm font-bold">
                        @if($systemContext === 'invoices')
                            {{ __('messages.transfer_system') ?? 'نظام الحوالات' }}
                        @else
                            {{ __('messages.bill_payments_system') ?? 'نظام الفواتير' }}
                        @endif
                    </span>
                </a>
    ...
    <div class="py-12" dir="{{ app()->getLocale() == 'ar' ? 'rtl' : 'ltr' }}">
        <div class="max-w-7xl mx-auto sm:px-6 lg:px-8 space-y-6" x-data="{ activeTab: '{{ $systemContext === 'invoices' ? 'bill_payments' : 'transfer' }}' }">
            ...
            <div x-show="activeTab === 'payments'" style="display: none;" x-transition:enter="transition ease-out duration-300" x-transition:enter-start="opacity-0 transform scale-95" x-transition:enter-end="opacity-100 transform scale-100">
                <livewire:customer.customer-payments />
            </div>
        </div>
    </div>
</x-app-layout>
