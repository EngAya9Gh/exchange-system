<div class="min-h-screen flex bg-[#f8f9fc] font-sans {{ app()->getLocale() == 'ar' ? 'text-right' : 'text-left' }}"
    dir="{{ app()->getLocale() == 'ar' ? 'rtl' : 'ltr' }}"
    x-data="{ rejectModal: false, rejectId: null, rejectNotes: '', mobileMenuOpen: false }" wire:init="autoSyncRates">
    <!-- Mobile overlay -->
    ...
    <!-- Modern Sidebar -->
    <aside
        :class="mobileMenuOpen ? 'translate-x-0' : 'translate-x-full rtl:translate-x-full ltr:-translate-x-full md:translate-x-0 md:rtl:translate-x-0 md:ltr:translate-x-0'"
        class="w-[280px] shrink-0 bg-white border-x border-slate-100 flex-col flex shadow-[4px_0_24px_rgba(0,0,0,0.02)] z-40 fixed md:sticky md:top-0 inset-y-0 right-0 rtl:right-0 ltr:left-0 transition-transform duration-300 ease-in-out h-screen">
        <!-- Logo Area -->
        ...
        <div
            class="px-8 text-xs font-bold text-slate-400 mb-2 uppercase tracking-wider flex justify-between items-center">
        ...
        <nav class="flex-1 px-4 py-2 space-y-1.5 overflow-y-auto">
            ...
            <button wire:click="$set('activeTab', 'new_transfer')"
                class="w-full flex items-center px-4 py-3.5 rounded-2xl transition-all {{ $activeTab === 'new_transfer' ? 'bg-primary-50 text-primary-600 font-bold' : 'text-slate-500 hover:bg-slate-50 hover:text-slate-800 font-semibold' }}">
                <svg class="w-5 h-5 ml-4 {{ $activeTab === 'new_transfer' ? 'text-primary-600' : 'text-slate-400' }}"
                    fill="none" stroke="currentColor" viewBox="0 0 24 24">
                    <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2"
            ...
            <button wire:click="$set('activeTab', 'ledger')"
                class="w-full flex items-center px-4 py-3.5 rounded-2xl transition-all {{ $activeTab === 'ledger' ? 'bg-primary-50 text-primary-600 font-bold' : 'text-slate-500 hover:bg-slate-50 hover:text-slate-800 font-semibold' }}">
                <svg class="w-5 h-5 ml-4 {{ $activeTab === 'ledger' ? 'text-primary-600' : 'text-slate-400' }}"
                    fill="none" stroke="currentColor" viewBox="0 0 24 24">
                    <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2"
            ...
            <button wire:click="$set('activeTab', 'rates')"
                class="w-full flex items-center px-4 py-3.5 rounded-2xl transition-all {{ $activeTab === 'rates' ? 'bg-primary-50 text-primary-600 font-bold' : 'text-slate-500 hover:bg-slate-50 hover:text-slate-800 font-semibold' }}">
                <svg class="w-5 h-5 ml-4 {{ $activeTab === 'rates' ? 'text-primary-600' : 'text-slate-400' }}"
                    fill="none" stroke="currentColor" viewBox="0 0 24 24">
                    <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2"
            ...
            @can('manage_users')
                <button wire:click="$set('activeTab', 'users')"
                    class="w-full flex items-center px-4 py-3.5 rounded-2xl transition-all {{ $activeTab === 'users' ? 'bg-primary-50 text-primary-600 font-bold' : 'text-slate-500 hover:bg-slate-50 hover:text-slate-800 font-semibold' }}">
                    <svg class="w-5 h-5 ml-4 {{ $activeTab === 'users' ? 'text-primary-600' : 'text-slate-400' }}"
                        fill="none" stroke="currentColor" viewBox="0 0 24 24">
                        <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2"
                ...
                <button wire:click="$set('activeTab', 'role_settings')"
                    class="w-full flex items-center px-4 py-3.5 rounded-2xl transition-all {{ $activeTab === 'role_settings' ? 'bg-primary-50 text-primary-600 font-bold' : 'text-slate-500 hover:bg-slate-50 hover:text-slate-800 font-semibold' }}">
                    <svg class="w-5 h-5 ml-4 {{ $activeTab === 'role_settings' ? 'text-primary-600' : 'text-slate-400' }}"
                        fill="none" stroke="currentColor" viewBox="0 0 24 24">
                        <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2"
                            d="M9 12l2 2 4-4m5.618-4.016A11.955 11.955 0 0112 2.944a11.955 11.955 0 01-8.618 3.04A12.02 12.02 0 003 9c0 5.591 3.824 10.29 9 11.622 5.176-1.332 9-6.03 9-11.622 0-1.042-.133-2.052-.382-3.016z">
                        </path>
                    </svg>
                    {{ __('messages.roles_and_permissions') }}
                </button>
            @endcan
            @can('manage_commissions')
                <!-- Transfer Commissions -->
                <button wire:click="$set('activeTab', 'transfer_commissions')"
                    class="w-full flex items-center px-4 py-3.5 rounded-2xl transition-all {{ $activeTab === 'transfer_commissions' ? 'bg-primary-50 text-primary-600 font-bold' : 'text-slate-500 hover:bg-slate-50 hover:text-slate-800 font-semibold' }}">
                    <svg class="w-5 h-5 ml-4 {{ $activeTab === 'transfer_commissions' ? 'text-primary-600' : 'text-slate-400' }}"
                        fill="none" stroke="currentColor" viewBox="0 0 24 24">
                        <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2"
                            d="M12 8c-1.657 0-3 .895-3 2s1.343 2 3 2 3 .895 3 2-1.343 2-3 2m0-8c1.11 0 2.08.402 2.599 1M12 8V7m0 1v8m0 0v1m0-1c-1.11 0-2.08-.402-2.599-1M21 12a9 9 0 11-18 0 9 9 0 0118 0z">
                        </path>
                    </svg>
                    {{ __('messages.transfer_commissions') }}
                </button>
            @endcan
            ...
            @role('Super Admin')
                ...
                <button wire:click="$set('activeTab', 'bill_payments')"
                    ...
                    {{ __('messages.bill_payments_system') }}
                </button>
                @can('manage_commissions')
                    <button wire:click="$set('activeTab', 'billing_commissions')"
                        class="w-full flex items-center px-4 py-3.5 rounded-2xl transition-all {{ $activeTab === 'billing_commissions' ? 'bg-primary-50 text-primary-600 font-bold' : 'text-slate-500 hover:bg-slate-50 hover:text-slate-800 font-semibold' }}">
                        <svg class="w-5 h-5 ml-4 {{ $activeTab === 'billing_commissions' ? 'text-primary-600' : 'text-slate-400' }}"
                            fill="none" stroke="currentColor" viewBox="0 0 24 24">
                            <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2"
                                d="M9 14l6-6m-5.5.5h.01m4.99 5h.01M19 21V5a2 2 0 00-2-2H7a2 2 0 00-2 2v16l3.5-2 3.5 2 3.5-2 3.5 2zM10 8.5a.5.5 0 11-1 0 .5.5 0 011 0zm5 5a.5.5 0 11-1 0 .5.5 0 011 0z">
                            </path>
                        </svg>
                        {{ __('messages.billing_commissions') }}
                    </button>
                @endcan
                <button wire:click="$set('activeTab', 'bill_payments_history')"
                    class="w-full flex items-center px-4 py-3.5 rounded-2xl transition-all {{ $activeTab === 'bill_payments_history' ? 'bg-primary-50 text-primary-600 font-bold' : 'text-slate-500 hover:bg-slate-50 hover:text-slate-800 font-semibold' }}">
                    <svg class="w-5 h-5 ml-4 {{ $activeTab === 'bill_payments_history' ? 'text-primary-600' : 'text-slate-400' }}"
                        fill="none" stroke="currentColor" viewBox="0 0 24 24">
                        <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2"
        ...
        <div class="p-6 mt-auto">
            <div
                class="bg-gradient-to-r from-primary-500 to-primary-600 rounded-[28px] p-6 text-white text-center shadow-soft-xl relative overflow-hidden">
                ...
                <div
                    class="w-10 h-10 bg-white rounded-full flex items-center justify-center mx-auto mb-3 shadow-sm relative z-10">
                ...
                <button
                    class="w-full py-2.5 bg-white text-primary-600 rounded-xl text-xs font-black shadow-sm relative z-10 transition hover:bg-gray-50">{{ __('messages.go_to_center') }}</button>
            </div>
        </div>
    </aside>
    ...
    <main class="flex-1 flex flex-col overflow-hidden relative">
        ...
        <header class="bg-transparent px-4 md:px-10 pt-4 md:pt-6 pb-2">
            <div class="flex items-center justify-between">
                ...
                <div class="hidden md:block">
                    <h1 class="text-[28px] font-black text-slate-800 tracking-tight flex items-center">
                        ...
                        @endif
                        @if($activeTab === 'new_transfer') <span
                        class="text-primary-600 ml-2">{{ __('messages.new_transfer') }}</span> @endif
                        @if($activeTab === 'ledger') <span
                        class="text-primary-600 ml-2">{{ __('messages.transactions') }}</span> @endif
                        @if($activeTab === 'rates') <span
                        class="text-primary-600 ml-2">{{ __('messages.exchange_rates') }}</span> @endif
                        @if($activeTab === 'users') <span
                        class="text-primary-600 ml-2">{{ __('messages.system_settings') }}</span> @endif
                        @if($activeTab === 'role_settings') <span class="text-primary-600 ml-2">{{ __('messages.roles_and_permissions') }}</span> @endif
                    ...
                    <p class="text-sm text-slate-400 font-medium mt-1">
                        @if($activeTab === 'dashboard')
                        {{ __('messages.hello_welcome_back', ['name' => auth()->user()->name]) }} @else
                        {{ __('messages.manage_details_here') }} @endif
                    </p>
                </div>
            ...
            <div class="md:hidden mt-6 mb-2 px-2">
                <h1 class="text-2xl font-black text-slate-800 tracking-tight flex items-center flex-wrap gap-1">
                    ...
                    @endif
                    @if($activeTab === 'new_transfer') <span
                    class="text-primary-600">{{ __('messages.new_transfer') }}</span> @endif
                    @if($activeTab === 'ledger') <span class="text-primary-600">{{ __('messages.transactions') }}</span>
                    @endif
                    @if($activeTab === 'rates') <span
                    class="text-primary-600">{{ __('messages.exchange_rates') }}</span> @endif
                    @if($activeTab === 'users') <span
                    class="text-primary-600">{{ __('messages.system_settings') }}</span> @endif
                    @if($activeTab === 'role_settings') <span class="text-primary-600">إدارة الأدوار والصلاحيات</span>
                    @endif
                    @if($activeTab === 'commissions') <span
        ...
        <div class="flex-1 overflow-y-auto px-4 md:px-10 py-6 pb-20">
            ...
            @if ($activeTab === 'dashboard')
                ...
                <div class="grid grid-cols-1 md:grid-cols-3 xl:grid-cols-5 gap-6 mb-8">
                    <!-- TRY Card -->
                    <div
                        class="bg-white rounded-[28px] p-6 flex flex-col items-center justify-between shadow-soft border border-slate-50 transition-transform hover:-translate-y-1">
                        ...
                        <div class="w-16 h-16 rounded-full bg-orange-50 flex items-center justify-center mb-4 relative">
                            ...
                            <div class="absolute -top-1 -right-1 bg-white rounded-full p-0.5">
                                <span class="flex h-3 w-3"><span
                                        class="animate-ping absolute inline-flex h-full w-full rounded-full bg-orange-400 opacity-75"></span><span
                                        class="relative inline-flex rounded-full h-3 w-3 bg-orange-500"></span></span>
                            </div>
                        </div>

                        <h3 class="text-2xl font-black text-slate-800 mb-1 tracking-tight">
                            {{ number_format($totalTrySent, 2) }}</h3>
                        <div class="text-[10px] font-bold text-orange-500 uppercase tracking-widest mt-4">
                            {{ __('messages.total_try_spent') }}</div>
                    </div>
                    ...
                    <div
                        ...
                        <h3 class="text-2xl font-black text-slate-800 mb-1 tracking-tight">
                            {{ number_format($totalUsdSent, 2) }}</h3>
                        <div class="text-[10px] font-bold text-emerald-500 uppercase tracking-widest mt-4">
                            {{ __('messages.total_usd_spent') }}</div>
                    </div>
                    ...
                    <div
                        ...
                        <div class="w-16 h-16 rounded-full bg-blue-50 flex items-center justify-center mb-4 relative">
                            <svg class="w-8 h-8 text-blue-400" fill="currentColor" viewBox="0 0 24 24">
                                <path
                                    d="M15 18.5c-2.51 0-4.68-1.42-5.76-3.5H15v-2H8.58c-.05-.33-.08-.66-.08-1s.03-.67.08-1H15v-2H9.24C10.32 6.92 12.5 5.5 15 5.5c1.61 0 3.09.59 4.23 1.57L21 4.24C19.41 2.85 17.3 2 15 2 10.3 2 6.36 5.25 5.28 9.5H3v2h2.06c-.04.33-.06.66-.06 1s.02.67.06 1H3v2h2.28C6.36 18.75 10.3 22 15 22c2.3 0 4.41-.85 6-2.24l-1.77-2.83C18.09 17.91 16.61 18.5 15 18.5z" />
                            </svg>
                        </div>

                        <h3 class="text-2xl font-black text-slate-800 mb-1 tracking-tight">
                            {{ number_format($totalEurSent, 2) }}</h3>
                        <div class="text-[10px] font-bold text-blue-500 uppercase tracking-widest mt-4">
                            {{ __('messages.total_eur_spent') }}</div>
                    </div>

                    @can('view_profits')
                        <!-- Profits / Commissions Card -->
                        <div
                            class="bg-gradient-to-br from-purple-500 to-indigo-600 rounded-[28px] p-6 flex flex-col justify-between shadow-soft border border-slate-50 transition-transform hover:-translate-y-1 relative overflow-hidden text-right">
                            <div class="absolute -left-10 -bottom-10 w-32 h-32 rounded-full bg-white/10 blur-2xl"></div>

                            <div>
                                <span
                                    class="text-xs text-white/80 font-bold uppercase tracking-wider">{{ __('messages.total_profits') }}</span>
                                <h3 class="text-3xl font-black text-white mt-2 tracking-tight">
                                    {{ number_format($totalCommissions, 2) }}</h3>
                            </div>

                            <div class="flex justify-between items-end mt-8 relative z-10">
                                <div
                                    class="text-[10px] font-bold text-white/90 uppercase tracking-widest bg-white/20 px-3 py-1.5 rounded-full backdrop-blur-sm">
                                    TRY</div>
                    ...
                    <!-- EGP Card (مقوم) -->
                    <div
                        class="bg-gradient-to-br from-primary-700 via-primary-600 to-rose-500 rounded-[28px] p-6 flex flex-col justify-between shadow-soft-xl relative overflow-hidden text-right">
                        ...
                        <div>
                            <span
                                class="text-xs text-white/80 font-bold uppercase tracking-wider">{{ __('messages.vodafone_balance') }}</span>
                            <h3 class="text-3xl font-black text-white mt-2 tracking-tight">
                                {{ number_format($totalEgpPaid, 2) }}</h3>
                        </div>

                        <div class="flex justify-between items-end mt-8 relative z-10">
                            <div
                                class="text-[10px] font-bold text-white/90 uppercase tracking-widest bg-white/20 px-3 py-1.5 rounded-full backdrop-blur-sm">
                                {{ __('messages.excellent_performance') }}</div>
                            <svg class="w-12 h-12 text-white/30" fill="currentColor" viewBox="0 0 24 24">
                                <path
                                    d="M16 4h2a2 2 0 0 1 2 2v14a2 2 0 0 1-2 2H6a2 2 0 0 1-2-2V6a2 2 0 0 1 2-2h2V2h2v2h4V2h2v2zM6 8v12h12V8H6zm2 3h8v2H8v-2zm0 4h5v2H8v-2z" />
                            </svg>
                        </div>
                    </div>
                </div>
                ...
                <div class="bg-white rounded-3xl p-6 shadow-soft border border-slate-50">
                    <div class="flex justify-between items-center mb-6">
                        ...
                        <button
                            class="px-4 py-2 bg-slate-50 hover:bg-slate-100 text-slate-600 rounded-xl text-xs font-bold transition">{{ __('messages.view_all') }}</button>
                    </div>

                    @if (session()->has('request_success'))
                        <div
                            class="mb-4 p-3 bg-emerald-50 text-emerald-800 text-sm font-bold rounded-xl border border-emerald-100 flex items-center">
                    ...
                    @if (session()->has('request_error'))
                        <div
                            class="mb-4 p-3 bg-rose-50 text-rose-800 text-sm font-bold rounded-xl border border-rose-100 flex items-center">
                            <svg class="w-5 h-5 ml-2" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                                <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M6 18L18 6M6 6l12 12">
                                </path>
                            </svg>
                                ...
                                @forelse($incomingRequests as $req)
                                    <tr class="border-b border-slate-50 hover:bg-slate-50/50 transition-colors group">
                                        <td class="px-4 py-4 text-center">
                                            <span
                                                class="text-xs font-bold text-slate-400">{{ $incomingRequests->firstItem() + $loop->index }}</span>
                                        </td>
                                        <td class="px-4 py-4">
                                            <div class="flex items-center">
                                                <div
                                                    class="w-10 h-10 rounded-full bg-slate-100 text-slate-500 flex items-center justify-center font-bold text-xs ml-3 group-hover:bg-primary-50 group-hover:text-primary-600 transition">
                                        ...
                                        <td class="px-4 py-4 font-black text-slate-800">
                                            {{ number_format((float) $req->amount, 2) }} <span
                                                class="text-xs text-slate-400 font-bold ml-1">{{ $req->currency }}</span>
                                        </td>
                                        <td class="px-4 py-4">
                                            <div
                                                class="text-xs font-bold text-slate-500 bg-slate-100 inline-block px-2.5 py-1 rounded-lg">
                                                {{ $req->created_at->format('M d, Y') }}
                                            </div>
                                        </td>
                                        <td class="px-4 py-4 text-center">
                                            <div class="flex flex-col gap-2 w-full max-w-[120px] mx-auto">
                                                <button wire:click="payTransfer({{ $req->id }})" wire:loading.attr="disabled"
                                                    class="w-full px-4 py-2 bg-emerald-50 hover:bg-emerald-500 text-emerald-600 hover:text-white rounded-xl text-xs font-bold transition disabled:opacity-75 disabled:cursor-wait flex items-center justify-center">
                                                    <span wire:loading.remove
                                                        wire:target="payTransfer({{ $req->id }})">{{ __('messages.receipt') }}</span>
                                                    <span wire:loading wire:target="payTransfer({{ $req->id }})"
                                                        class="flex items-center">
                                                        <svg class="animate-spin ml-1 h-4 w-4"
                                                ...
                                                <button x-on:click="rejectId = {{ $req->id }}; rejectModal = true"
                                                    class="w-full px-4 py-2 bg-rose-50 hover:bg-rose-500 text-rose-600 hover:text-white rounded-xl text-xs font-bold transition flex items-center justify-center">
                                                    {{ __('messages.reject') }}
                                                </button>
                                            </div>
                                        </td>
            ...
            @if ($activeTab === 'new_transfer')
                <div class="grid grid-cols-1 lg:grid-cols-3 gap-6">
                    ...
                    <div class="lg:col-span-2 space-y-6">
                        <div
                            class="bg-white rounded-[28px] shadow-soft border border-slate-50 p-8 relative overflow-hidden">
                            <!-- Decorative subtle gradient at top right -->
                            <div
                                class="absolute top-0 right-0 w-40 h-40 bg-gradient-to-bl from-primary-50 to-transparent rounded-bl-full -z-0 opacity-50">
                            </div>
                            ...
                            @if (session()->has('transfer_success'))
                                <div
                                    class="mb-8 p-4 bg-emerald-50 text-emerald-800 font-bold rounded-2xl flex items-center relative">
                                    <svg class="w-5 h-5 ml-2 text-emerald-500" fill="none" stroke="currentColor"
                                        viewBox="0 0 24 24">
                                        <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2"
                            ...
                            <form x-data="{ confirming: false }" @submit.prevent="confirming = true"
                                ...
                                <div class="bg-slate-50/50 p-6 rounded-[24px] space-y-5">
                                    ...
                                    <div class="grid grid-cols-1 md:grid-cols-2 gap-5">
                                        <div>
                                            <label
                                                class="block text-sm font-bold text-slate-500 mb-2 uppercase tracking-wider">{{ __('messages.recipient_name') }}</label>
                                            <input wire:model="recipient_name" type="text"
                                                class="w-full bg-white border-none text-slate-800 font-bold rounded-xl focus:ring-2 focus:ring-primary-500 px-4 py-3.5 shadow-sm transition"
                                                required placeholder="{{ __('messages.full_name') }}">
                                            @error('recipient_name') <span
                                                class="text-xs text-rose-500 font-bold mt-1 block">{{ $message }}</span>
                                            @enderror
                                        </div>
                                        <div>
                                            <label
                                                class="block text-sm font-bold text-slate-500 mb-2 uppercase tracking-wider">{{ __('messages.recipient_phone') }}</label>
                                            <input wire:model="recipient_phone" type="text"
                                                class="w-full bg-white border-none text-slate-800 font-bold rounded-xl focus:ring-2 focus:ring-primary-500 px-4 py-3.5 shadow-sm transition"
                                                required placeholder="05xxxxxxxx">
                                            @error('recipient_phone') <span
                                                class="text-xs text-rose-500 font-bold mt-1 block">{{ $message }}</span>
                                            @enderror
                                        </div>
                                    </div>
                                    ...
                                    <div class="grid grid-cols-1 md:grid-cols-2 gap-5">
                                        <div>
                                            <label
                                                class="block text-sm font-bold text-slate-500 mb-2 uppercase tracking-wider">{{ __('messages.destination') }}</label>
                                            <select wire:model="destination"
                                                class="w-full bg-white border-none text-slate-800 font-bold rounded-xl focus:ring-2 focus:ring-primary-500 pr-4 pl-10 py-3.5 shadow-sm transition bg-left"
                                                required>
                                                <option value="{{ __('messages.all_governorates_vodafone') }}">
                                                    {{ __('messages.all_governorates_vodafone') }}</option>
                                            </select>
        ...
        <!-- TAB: Transfer Commissions -->
        @if ($activeTab === 'transfer_commissions')
            @can('manage_commissions')
                <livewire:admin.transfer-commissions />
            @endcan
        @endif

        <!-- TAB: Billing Commissions -->
        @if ($activeTab === 'billing_commissions')
            @can('manage_commissions')
                <livewire:admin.billing-commissions />
            @endcan
        @endif
