    ...
    <aside
        ...
        <nav class="flex-1 px-4 py-2 space-y-1.5 overflow-y-auto">
            ...
            @can('manage_commissions')
                <button wire:click="$set('activeTab', 'commissions')"
                    class="w-full flex items-center px-4 py-3.5 rounded-2xl transition-all {{ $activeTab === 'commissions' ? 'bg-primary-50 text-primary-600 font-bold' : 'text-slate-500 hover:bg-slate-50 hover:text-slate-800 font-semibold' }}">
                    <svg class="w-5 h-5 ml-4 {{ $activeTab === 'commissions' ? 'text-primary-600' : 'text-slate-400' }}"
                        fill="none" stroke="currentColor" viewBox="0 0 24 24">
                        <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2"
            ...
            <div class="flex items-center justify-between">
                ...
                <div class="hidden md:block">
                    <h1 class="text-[28px] font-black text-slate-800 tracking-tight flex items-center">
                        ...
                        @if($activeTab === 'role_settings') <span class="text-primary-600 ml-2">إدارة الأدوار
                        والصلاحيات</span> @endif
                        @if($activeTab === 'commissions') <span
                        class="text-primary-600 ml-2">{{ __('messages.commissions') }}</span> @endif
                        @if($activeTab === 'deposit_requests') <span class="text-primary-600 ml-2">طلبات شحن
                        الرصيد</span> @endif
                        @if($activeTab === 'balance_management') <span class="text-primary-600 ml-2">إدارة الأرصدة
                        والسقوف</span> @endif
                    </h1>
