<div class="bg-white rounded-2xl shadow-sm border border-slate-100 p-6 mb-6">
    <div class="flex items-center justify-between">
        <div class="flex items-center gap-4">
            ...
            <div>
                <h3 class="text-lg font-bold text-slate-800">{{ __('messages.telegram_notifications') }}</h3>
                <p class="text-sm text-slate-500 mt-1">{{ __('messages.telegram_link_desc') }}</p>
            </div>
        </div>
        ...
        <div>
            ...
            @else
                <div class="flex items-center gap-3">
                    ...
                    <button wire:click="unlink" class="text-xs font-bold text-red-500 hover:text-red-700 transition">{{ __('messages.unlink_account') }}</button>
                </div>
            @endif
        </div>
    </div>
