...
<div>
    <div class="mb-8 text-center">
        ...
        <p class="text-sm text-slate-500 mt-2">انضم إلى شبكة Teacher VC للحوالات </p>
    </div>
    ...
    <form wire:submit="register" action="{{ route('register') }}" method="POST" onsubmit="event.preventDefault();">
        ...
        <div>
            ...
            <input wire:model="name" id="name"
                class="block mt-2 w-full bg-slate-50 text-slate-800 font-semibold rounded-xl border-none focus:ring-2 focus:ring-primary-500 px-4 py-3.5 transition"
        ...
        <div class="mt-8">
            <button type="submit"
                class="w-full py-4 bg-gradient-to-r from-primary-600 to-rose-600 hover:from-primary-700 hover:to-rose-700 text-white rounded-xl font-black text-lg shadow-soft transition-transform hover:-translate-y-1">
                {{ __('إنشاء الحساب') }}
            </button>
        </div>

        <div class="mt-6 text-center">
            <p class="text-sm text-slate-500 font-bold">لديك حساب بالفعل؟ <a href="{{ route('login') }}"
                    class="text-primary-600 hover:text-primary-700 transition" wire:navigate>تسجيل الدخول</a></p>
        </div>
    </form>
</div>